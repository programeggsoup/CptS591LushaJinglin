...
'''old_mod=g.modularity(g.clusters().membership)'''
...
while True:
    ...
    for i in edge_list:
        '''record old modularity'''
        old_mod=g.modularity(g.clusters().membership)
        '''delete this edge'''
        g.delete_edges([i])
        '''calculate the new modularity'''
        new_mod = g.modularity(g.clusters().membership)
        '''add the delta value to deltamod list'''
        deltamod.append(new_mod-old_mod)
        '''add this edge back to the graph'''
        g.add_edges([i])
        
    maxdeltamod = max(deltamod)
    '''mindeltamod = min(deltamod)'''
    maxdeltamodindex=deltamod.index(maxdeltamod)
    '''print edgeset[maxdeltamodindex].tuple'''
    '''print g.ecount()'''
    '''print g.clusters()'''
    '''print mindeltamod'''
    ...
    '''find the edge with largest modularity'''
    '''g.delete_edges([edgeset[maxdeltamodindex].tuple])'''
    
    '''compute new modulartiy'''
    new_mod = g.modularity(g.clusters().membership)
